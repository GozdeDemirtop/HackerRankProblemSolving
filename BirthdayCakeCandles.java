...
public class BirthdayCakeCandles {
    ...
    static int birthdayCakeCandles(int[] ar) {
        ...
        int maxHeightCandle = Integer.MIN_VALUE;
        int countMaxHeightCandle = 0;

        for(int i=0; i<ar.length; i++){
            if(ar[i] == maxHeightCandle){
                countMaxHeightCandle++;
            }
            if(ar[i]>maxHeightCandle){
                maxHeightCandle=ar[i];
                countMaxHeightCandle=1;
            }
        }
        return countMaxHeightCandle;
    }
